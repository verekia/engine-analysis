# Wren — A Minimal High-Performance 3D Engine
...
│   │   ├── types.ts      # Shared GPU types (Pipeline, BindGroup, Buffer, etc.)
│   │   ├── webgl2.ts     # WebGL2 backend
│   │   └── webgpu.ts     # WebGPU backend
│   ├── renderer.ts       # Sort-based draw-call renderer
│   ├── scene.ts          # Scene graph (nodes, groups, transforms)
│   ├── camera.ts         # Perspective/orthographic cameras
│   ├── math/             # Vec3, Mat4, Quat (z-up, RH)
│   │   ├── vec3.ts
│   │   ├── mat4.ts
│   │   ├── quat.ts
│   │   └── aabb.ts
│   ├── geometry/         # Parametric + loaded geometries
│   │   ├── primitives.ts # Plane, box, sphere, cone, cylinder, capsule, circle
│   │   ├── geometry.ts   # Geometry container (typed arrays, vertex layout)
│   │   └── skinned.ts    # Skinned geometry with bone indices/weights
│   ├── material/
│   │   ├── basic.ts      # Unlit material
│   │   ├── lambert.ts    # Diffuse-lit material
│   │   └── shaders/      # GLSL/WGSL shader sources
│   ├── texture/
│   │   ├── texture.ts    # Texture abstraction
│   │   ├── ktx2.ts       # KTX2/Basis transcoder integration
│   │   └── loader.ts     # Image/texture loading utilities
│   ├── light/
│   │   ├── directional.ts
│   │   ├── ambient.ts
│   │   └── shadow.ts     # CSM implementation
│   ├── animation/
│   │   ├── clip.ts       # Animation clip (keyframes)
│   │   ├── mixer.ts      # Animation mixer with crossfade
│   │   └── skeleton.ts   # Skeleton, bones, skinning matrices
│   ├── postfx/
│   │   ├── bloom.ts      # Unreal-style bloom pipeline
│   │   └── msaa.ts       # MSAA resolve
│   ├── transparency/
│   │   └── wboit.ts      # Weighted Blended OIT
│   ├── raycasting/
│   │   ├── bvh.ts        # SAH-based flat-array BVH
│   │   └── raycaster.ts  # Ray-scene intersection
│   └── culling/
│       └── frustum.ts    # Frustum culling
├── extras/
│   ├── controls/
│   │   └── orbit.ts      # Orbit camera controls
│   ├── loaders/
│   │   ├── gltf.ts       # glTF 2.0 loader
│   │   └── draco.ts      # Draco WASM decoder integration
│   └── html-overlay.ts   # CSS3D-style DOM overlay
├── react/
│   ├── reconciler.ts     # Custom React reconciler
│   ├── canvas.ts         # <WrenCanvas> root component
│   ├── hooks.ts          # useFrame, useWren, useLoader
│   └── components.ts     # JSX element catalog
...
1. [Hardware Abstraction Layer](architecture/01-hal.md) — WebGL2/WebGPU unified device API
2. [Sort-Based Renderer](architecture/02-renderer.md) — Render keys, sorting, state caching, draw submission
3. [Scene Graph](architecture/03-scene-graph.md) — Nodes, groups, transforms, bone attachment
4. [Materials](architecture/04-materials.md) — Basic, Lambert, vertex colors, material index system, emissive
5. [Geometry](architecture/05-geometry.md) — Parametric primitives, vertex layout, GLTF, Draco
6. [Textures](architecture/06-textures.md) — Color, AO, KTX2/Basis Universal
7. [Lighting & Shadows](architecture/07-lighting.md) — Directional, ambient, 3-cascade CSM
8. [Skeletal Animation](architecture/08-animation.md) — Clips, mixer, crossfade, bone attachment
9. [Bloom](architecture/09-bloom.md) — Per-vertex emissive, Unreal-style post-process bloom
10. [Transparency](architecture/10-transparency.md) — Weighted Blended OIT
11. [Raycasting](architecture/11-raycasting.md) — SAH BVH, flat-array traversal
12. [Culling & MSAA](architecture/12-culling.md) — Frustum culling, MSAA, performance strategies
13. [Controls & HTML Overlay](architecture/13-controls.md) — Orbit controls, DOM overlay
14. [React Bindings](architecture/14-react.md) — Reconciler, hooks, JSX catalog
15. [Public API](architecture/15-api.md) — Entry points, usage examples, TypeScript types
...
- No classes for data — plain objects + typed arrays
- Classes only for API-facing types with clear ownership (Scene, Renderer, Mesh)
- Minimal abstractions — three similar lines > one premature helper
